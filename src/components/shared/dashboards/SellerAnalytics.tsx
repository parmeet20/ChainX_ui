...
const CustomerAnalyticsComponent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setIsLoading(true);
      try {
        const analyticsData: ICustomerAnalytics = await getCustomerAnalytics(
          program,
          publicKey
        );
        ...
        if (
          analyticsData &&
          analyticsData.transactions_sent !== undefined &&
          analyticsData.transactions_recieved !== undefined
        ) {
          setAnalytics(analyticsData);
        } else {
          console.warn(
            "Received incomplete or invalid customer analytics data:",
            analyticsData
          );
          setAnalytics(null);
  ...
  const totalTransactions = useMemo(() => {
    ...
    const sentCount = Array.isArray(analytics.transactions_sent)
      ? analytics.transactions_sent.length
      : 0;
    const receivedCount = Array.isArray(analytics.transactions_recieved)
      ? analytics.transactions_recieved.length
      : 0;
    ...
    const processTransactions = (
      ...
      transactions.forEach((t) => {
        ...
        if (
          ...
          console.warn(
            "Skipping transaction with missing timestamp or amount:",
            t
          );
          return;
        }
  ...
  return (
    ...
    >
      ...
      <div className="min-h-[300px] flex items-center justify-center">
        {" "}
        {/* Ensure min height and center content */}
